  ...
  createTest: function(name, config) {
    ...
    var test = function(testGroupName, functions, that) {
      ...
      if (functions.length !== testData.length)
        throw Error(
          "The number of functions does not match the number of test groups"
        );

      var pluckedValues = _.pluck(testData, "name");
      var index = pluckedValues.indexOf(testGroupName);

      if (index === -1) throw Error("Test Group not found");
  ...
  var tests = _.map(config, function(test) {
    if (test.name && typeof test.name === "string") {
      return {
        // Weight from 0..1
        weight:
          (_.isFinite(test.weight) ? test.weight : defaultWeight) / totalWeight,
        name: test.name
      };
    }
  }).sort(function(a, b) {
